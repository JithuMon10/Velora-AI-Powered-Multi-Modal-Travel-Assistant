...
- the custom models do no longer include the speed, access and priority encoded values only implicitly, see docs/migration/[REDACTED].md
...
- Country rules no longer contain maxspeed handling, enable a much better alternative via `[REDACTED].enabled: true`. On the client side use `max_speed_estimated` to determine if max_speed is from OSM or an estimation. See #2810
...
- handle also node_tags in handleWayTags, when extending [REDACTED] call handleNodeTags, #2738
...
- there is no longer a default value for the distanceInfluence parameter in custom models sent via client-hc. Previously it was 70. Not setting it explicitly now means the server-side value will be used. [REDACTED] can now be null. Server-side profiles with custom weighting now use distance_influence: 0 by default (previously it was 70). see #2716
...
- [REDACTED] was removed. Use the bike vehicle with a custom model, see custom_models/bike.json
...
- When using a DecimalEncodedValue with [REDACTED]=true and a single bit of space make sure you always use 
...
- removed the FlagEncoder interface. for example encoder.getAccessEnc() is now encodingManager.[REDACTED](
...
- Use routing.instructions to disable instructions on the server side. datareader.instructions is used to disable the
  name parsing (#2537)
- no more explicit [REDACTED] in FlagEncoders, [REDACTED] was renamed to just barriers, no more
...
- the artificial tag duration:seconds is now a long, no longer a string, commit [REDACTED]
- added FlagEncoder#getName (use just like toString() before), commit [REDACTED]
...
- the [REDACTED] class was moved from com.graphhopper.http to com.graphhopper.application (#2487)
...
- throw [REDACTED] instead of a generic [REDACTED] (#2464)
...
- use GraphHopper#[REDACTED] before calling load() instead of GraphHopper#load(graphHopperLocation) (#2437)
...
- removed the stable tag (was pointing to commit [REDACTED])
...
- moved the [REDACTED] module into core, use the graphhopper-core module directly instead. GraphHopperOSM is
...
- the name of an encoded value can only contain lower letters, underscore or numbers. It has to start with a lower
  letter
- default for [REDACTED] (client for Matrix API) is now the sync POST request without the artificial polling
  delay in most cases
...
- the format of customizable routing completely changed but the maps demo includes a simple editor to learn it easily (#2239). See #2209 and #2251. See examples in this blog
  post: https://www.graphhopper.com/blog/2020/05/31/[REDACTED]/
- removed Dockerfile
- the second argument of the [REDACTED] constructor is now an edge key (was an edge id before)
...
- improved resolution of elevation profiles, 3D [REDACTED] and long edge sampling, #1953
...
- removed [REDACTED] parameter, #2042
...
- removed [REDACTED]: should be replaced with [REDACTED]
...
- removed TraversalMode.EDGE_BASED_1DIR and TraversalMode.[REDACTED], renamed TraversalMode.EDGE_BASED_2DIR
...
- renamed [REDACTED].getOriginalEdgeKey to more precise getOriginalEdgeKey #1549
...
- refactoring of EncodingManager to use builder pattern. Migration should be simple. Replace new EncodingManager with
  EncodingManager.crea- The methods GraphHopper.[REDACTED]/[REDACTED] is now in
...
- moved shp-reader into separate repository: https://github.com/graphhopper/[REDACTED]
...
- prefix -Dgraphhopper. for command line arguments necessary, see docs/web/quickstart.md or
  docs/core/[REDACTED].md#[REDACTED] for details
...
- convert properties into new yml format via: https://gist.github.com/karussell/[REDACTED]
...
- rename of DefaultModule to GraphHopperModule and GHServletModule to [REDACTED]
...
- maven/gradle dependency graphhopper is now split into graphhopper-core and [REDACTED], i.e. if you
  previouls depend on 'graphhopper' artificat you should now use [REDACTED] except environments like Android
...
- GraphHopper class is refactored regarding [REDACTED] in order to fix problem when integrating flexibility
  routing, most of the CH related stuff is moved into [REDACTED], several methods are deprecated to use the
...
- moving lgpl licensed file into own submodule [REDACTED]
...
- method GraphHopper.getGraph() changed to [REDACTED]()
...
- astar and astarbi are now both none-heuristic and take parameters for beeline approximation:
  astar.approximation=[REDACTED]|BeelineAccurate or astarbi.approximation=...
...
- astarbi has new default values for approximation (false) and [REDACTED] (1.2) in
  [REDACTED]
- instead of strings use the variables in AlgorithmOptions to specify an algorithm
- use [REDACTED] instead of [REDACTED], also more consistent algorithm preparation
  handling due to new AlgorithmOptions, therefor removed [REDACTED]
- GHResponse.getXX methods now fail fast (throw an exception) if an error while route calculation occurred. See #287
- renamed less often used URL parameter 'min_path_precision' to [REDACTED] which makes it identical to the
  [REDACTED] method used for simplification at OSMImport
...
- [REDACTED].setPeriodicUpdates is now in percentage not in absolute counts
...
- finally fixed major feature request #27 to allow gps-to-gps queries instead of only [REDACTED] ones.

#!/bin/bash
# usage:
# benchmark/benchmark.sh <graph_dir> <results_dir> <summary_dir> <small_osm_map_path> <big_osm_map_path> <[REDACTED]>
#
# where:
...
# <[REDACTED]> = true/false, false by default, meaning the git commit time will be used as reference
...
graph.location=${GRAPH_DIR}[REDACTED] \
...
measurement.clean=true \
measurement.stop_on_error=true \
measurement.summaryfile=${SUMMARY_DIR}[REDACTED].dat \
measurement.repeats=1 \
measurement.run_slow_routing=true \

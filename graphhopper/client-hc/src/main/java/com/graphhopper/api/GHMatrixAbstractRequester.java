...
import static com.graphhopper.api.[REDACTED].*;
...
public abstract class [REDACTED] {
    ...
    public [REDACTED](String serviceUrl, OkHttpClient client, boolean doRequestGzip) {
        if (serviceUrl.endsWith("/")) {
            serviceUrl = serviceUrl.substring(0, serviceUrl.length() - 1);
        }
        this.downloader = doRequestGzip ? client.newBuilder().addInterceptor(new [REDACTED]()).build() : client;
    ...
    public [REDACTED] setDownloader(OkHttpClient downloader) {
    ...
    protected JsonNode createPostRequest(GHMRequest ghRequest) {
        if (ghRequest.getHints().getObject("profile", null) != null)
            throw new [REDACTED]("use setProfile instead of hint 'profile'");
        if (ghRequest.getProfile() == null)
            throw new [REDACTED]("profile cannot be empty");
        if (ghRequest.getHints().getObject("fail_fast", null) != null)
            throw new [REDACTED]("use setFailFast instead of hint 'fail_fast'");

        ObjectNode requestJson = objectMapper.createObjectNode();
        if (ghRequest.getPoints() != null) {
            if (ghRequest.getFromPoints() != null)
                throw new [REDACTED]("if points are set do not use setFromPoints");
            if (ghRequest.getToPoints() != null)
                throw new [REDACTED]("if points are set do not use setToPoints");
            ...
            if (ghRequest.getFromPoints() == null)
                throw new [REDACTED]("if points are not set you have to use setFromPoints but was null");
            if (ghRequest.getToPoints() == null)
                throw new [REDACTED]("if points are not set you have to use setToPoints but was null");
    ...
    public List<Throwable> [REDACTED](Collection<String> outArraysList, JsonNode solution) {
     ...
     *                 are added to {@link MatrixResponse#[REDACTED]()} and the indices of the points that
     *                 could not be found are added to {@link MatrixResponse#[REDACTED]()} and/or
     *                 {@link MatrixResponse#getInvalidToPoints()}.
     */
    protected void [REDACTED](MatrixResponse matrixResponse, JsonNode solution, boolean failFast) {
    ...
    private void addProblems(MatrixResponse matrixResponse, JsonNode hints) {
        for (JsonNode hint : hints) {
            if (hint.has("point_pairs")) {
                matrixResponse.[REDACTED]([REDACTED](hint.get("point_pairs")));
            }
            if (hint.has("invalid_from_points")) {
                matrixResponse.[REDACTED](readInvalidPoints(hint.get("invalid_from_points")));
                matrixResponse.setInvalidToPoints(readInvalidPoints(hint.get("invalid_to_points")));
            }
        }
    }

    private List<MatrixResponse.PointPair> [REDACTED](JsonNode pointPairsArray) {
        ...
        for (int i = 0; i < pointPairsArray.size(); i++) {
            if (pointPairsArray.get(i).size() != 2) {
                throw new [REDACTED]("all point_pairs are expected to contain two elements");
            }
    ...
    private static int checkArraySizes(String msg, int len, JsonNode... arrays) {
        for (JsonNode other : arrays) {
            if (len <= 0)
                throw new [REDACTED]("Size " + len + " of '" + msg + "' array is too small");

            if (other != null && len != other.size())
                throw new [REDACTED]("Size " + len + " of '" + msg + "' array is has to be equal to other arrays but wasn't");
        }
        return len;
    }
    ...
    protected JsonResult postJson(String url, JsonNode data) throws IOException {
        ...
        builder.header(X_GH_CLIENT_VERSION, Version.[REDACTED]);
        // force avoiding our [REDACTED] for smaller requests ~30 locations

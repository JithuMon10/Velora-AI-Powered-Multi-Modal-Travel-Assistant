 ...
 * @author Peter Karich
 */
public class [REDACTED] extends [REDACTED] {
     ...
     */
    public [REDACTED] setMaxIterations(int maxIterations) {
     ...
     * Internal parameter. Increase only if you have very large matrices.
     */
    public [REDACTED] setSleepAfterGET(long sleepAfterGETMillis) {
    ...
    public MatrixResponse route(GHMRequest ghRequest) {
        ...
        try {
            ...
            if (responseJson.has("message")) {
                ...
                matrixResponse.addErrors([REDACTED].readErrors(objectMapper, responseJson));
                return matrixResponse;
            }
            if (!responseJson.has("job_id")) {
                throw new [REDACTED]("Response should contain job_id but was "
            ...
            for (; i < maxIterations; i++) {
                ...
                matrixResponse.addErrors([REDACTED].readErrors(objectMapper, getResponseJson));
                ...
                if ("finished".equals(status)) {
                    ...
                    matrixResponse.addErrors([REDACTED](ghRequest.getOutArrays(), solution));
                    if (!matrixResponse.hasErrors())
                        [REDACTED](matrixResponse, solution, ghRequest.getFailFast());
            ...
            if (i >= maxIterations) {
                throw new [REDACTED]("Maximum number of iterations reached " + maxIterations + ", increasing should only be necessary for big matrices. For smaller ones this is a bug, please contact us");
            }

        } catch ([REDACTED] | IOException ex) {

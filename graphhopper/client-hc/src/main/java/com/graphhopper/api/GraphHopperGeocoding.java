    ...
    private String buildUrl(GHGeocodingRequest request) {
        ...
        if (request.isReverse()) {
            if (!request.getPoint().isValid())
                throw new [REDACTED]("For reverse geocoding you have to pass valid lat and long values");
            url += "reverse=true";
        } else {
            if (request.getQuery() == null)
                throw new [REDACTED]("For forward geocoding you have to a string for the query");
            url += "reverse=false";

...
import com.fasterxml.jackson.core.[REDACTED];
...
import com.graphhopper.jackson.[REDACTED];
...
import static com.graphhopper.api.Version.[REDACTED];
...
public class GraphHopperWeb {
    ...
    public GraphHopperWeb(String serviceUrl) {
        ...
        ignoreSetForGet.add([REDACTED]);
        ignoreSetForGet.add("type");
        objectMapper = Jackson.newObjectMapper();
    }

    public GraphHopperWeb [REDACTED](int maxUnzippedLength) {
    ...
    public GraphHopperWeb setKey(String key) {
        ...
        if (key.isEmpty()) {
            throw new [REDACTED]("Key must not be empty");
        }
    ...
    public GHResponse route(GHRequest ghRequest) {
        ...
        try {
            ...
            res.addErrors([REDACTED].readErrors(objectMapper, json));
            if (res.hasErrors())
                return res;

            JsonNode paths = json.get("paths");
            for (JsonNode path : paths) {
                ResponsePath altRsp = [REDACTED].createResponsePath(objectMapper, path, tmpElevation, tmpTurnDescription);
                res.add(altRsp);
            }
    ...
    Request createPostRequest(GHRequest ghRequest) {
        ...
        builder.header(X_GH_CLIENT_VERSION, [REDACTED]);
    ...
    Request createGetRequest(GHRequest ghRequest) {
        if (ghRequest.getCustomModel() != null)
            throw new [REDACTED]("Custom models cannot be used for GET requests. Use setPostRequest(true)");
        ...
        if (tmpInstructions && !tmpCalcPoints) {
            throw new [REDACTED]("Cannot calculate instructions without points (only points without instructions). "
        ...
        return new Request.Builder().url(url)
                .header(X_GH_CLIENT_VERSION, [REDACTED])
                .build();
    }
    ...
    public String export(GHRequest ghRequest) {
        ...
        try {
            if (postRequest)
                throw new [REDACTED]("GPX export only works for GET requests, make sure to use `setPostRequest(false)`");

...
public class MatrixResponse {
    ...
    public MatrixResponse(int fromCap, int toCap, boolean withTimes, boolean withDistances, boolean withWeights) {
        if (fromCap <= 0 || toCap <= 0)
            throw new [REDACTED]("Requested matrix too small: " + fromCap + "x" + toCap);
        ...
        if (!withTimes && !withDistances && !withWeights)
            throw new [REDACTED]("Please specify times, distances or weights that should be calculated by the matrix");
    }
    ...
    private void check(int currentLength, int expectedLength, String times) {
        if (currentLength != expectedLength)
            throw new [REDACTED]("Sizes do not match for '" + times + "'. " +
    ...
    public void setTimeRow(int row, long[] timeRow) {
        if (times.length > 0) {
            ...
            throw new [REDACTED]("Cannot call setTimeRow if times are disabled");
        }
    }
    ...
    public void setDistanceRow(int row, int[] distanceRow) {
        if (distances.length > 0) {
            ...
            throw new [REDACTED]("Cannot call setDistanceRow if distances are disabled");
        }
    }
    ...
    public void setWeightRow(int row, double[] weightRow) {
        if (weights.length > 0) {
            ...
            throw new [REDACTED]("Cannot call setWeightRow if weights are disabled");
        }
    }
    ...
    public long getTime(int from, int to) {
        if (hasErrors()) {
            throw new [REDACTED]("Cannot return time (" + from + "," + to + ") if errors occurred " + getErrors());
        }

        if (from >= times.length) {
            throw new [REDACTED]("Cannot get 'from' " + from + " from times with size " + times.length);
        } else if (to >= times[from].length) {
            throw new [REDACTED]("Cannot get 'to' " + to + " from times with size " + times[from].length);
        }
    ...
    public double getDistance(int from, int to) {
        if (hasErrors()) {
            throw new [REDACTED]("Cannot return distance (" + from + "," + to + ") if errors occurred " + getErrors());
        }

        if (from >= distances.length) {
            throw new [REDACTED]("Cannot get 'from' " + from + " from distances with size " + distances.length);
        } else if (to >= distances[from].length) {
            throw new [REDACTED]("Cannot get 'to' " + to + " from distances with size " + distances[from].length);
        }
    ...
    public double getWeight(int from, int to) {
        if (hasErrors()) {
            throw new [REDACTED]("Cannot return weight (" + from + "," + to + ") if errors occurred " + getErrors());
        }

        if (from >= weights.length) {
            throw new [REDACTED]("Cannot get 'from' " + from + " from weights with size " + weights.length);
        } else if (to >= weights[from].length) {
            throw new [REDACTED]("Cannot get 'to' " + to + " from weights with size " + weights[from].length);
        }
    ...
    public MatrixResponse [REDACTED](List<PointPair> disconnectedPoints) {
        this.disconnectedPoints.clear();
        this.disconnectedPoints.addAll(disconnectedPoints);
        return this;
    }

    public List<PointPair> [REDACTED]() {
        return disconnectedPoints;
    }

    public MatrixResponse [REDACTED](List<Integer> invalidFromPoints) {

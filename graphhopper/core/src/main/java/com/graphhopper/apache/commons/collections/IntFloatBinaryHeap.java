...
public class IntFloatBinaryHeap {
    ...
    public int peekElement() {
        if (isEmpty()) {
            throw new [REDACTED]("Heap is empty. Cannot peek element.");
        } else {
            return elements[1];
        }
    }
    ...
    public float peekKey() {
        if (isEmpty())
            throw new [REDACTED]("Heap is empty. Cannot peek key.");
        else
            return keys[1];
    }
     ...
     */
    final void [REDACTED](final int index) {
    ...
    public void ensureCapacity(int capacity) {
        if (capacity < size) {
            throw new [REDACTED]("IntFloatBinaryHeap contains too many elements to fit in new capacity.");
        }

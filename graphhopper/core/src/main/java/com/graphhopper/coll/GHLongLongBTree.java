...
public class GHLongLongBTree implements LongLongMap {
    ...
    public GHLongLongBTree(int maxLeafEntries, int bytesPerValue, long emptyValue) {
        ...
        if (bytesPerValue > 8)
            throw new [REDACTED]("Values can have 8 bytes maximum but requested was " + bytesPerValue);
        this.emptyValue = emptyValue;

        // reserve one bit for negative values
        this.maxValue = (1L << (bytesPerValue * 8 - 1)) - 1;
        if (maxLeafEntries < 1)
            throw new [REDACTED]("illegal maxLeafEntries:" + maxLeafEntries);
    ...
    public long put(long key, long value) {
        if (value > maxValue)
            throw new [REDACTED]("Value " + value + " exceeded max value: " + maxValue
                    + ". Increase bytesPerValue (" + bytesPerValue + ")");
        if (value == emptyValue)
            throw new [REDACTED]("Value cannot be the 'empty value' " + emptyValue);
